...
class PatioDesign extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            file: '', patioPhoto: '', patioAllImages: [], patioImages: [],
            patioCount: [], patioAllCount: [], patioLikes: [], patioAllLikes: []
        }
    }
    ...
    setDesignDetails = (result) => {
            ...
            if (val.patio != null) {
                ...
                if (val.userName === this.props.userName) {
                    ...
                    val.patio.map((design, id) => {
                        if (design.patioImage !== null && design.patioImage !== undefined) {
                            this.state.patioImages.push(design.patioImage);
                            if (id === 0) {
                                this.state.patio[0].patioImage = design.patioImage;
                            }
                        }
                        if (design.patioCount !== null && design.patioCount !== undefined) {
                            this.state.patioCount.push(design.patioCount);
                            if (id === 0) {
                                this.state.patio[0].patioCount = design.patioCount;
                            }
                        }
                        if (design.patioLikes !== null && design.patioLikes !== undefined) {
                            this.state.patioLikes.push(design.patioLikes);
                            if (id === 0) {
                                this.state.patio[0].patioLikes = design.patioLikes;
                            }
                        }
                        if (id > 0) {
                            this.state.patio = [...this.state.patio, design];
                        }
                    })
                }
                val.patio.map((design, id) => {
                    if (design !== null && design !== undefined) {
                        this.state.patioAllImages.push(design.patioImage);
                    }
                    if (design !== null && design !== undefined) {
        ...
        this.setState({
            patioAllImages: this.state.patioAllImages,
            patioAllCount: this.state.patioAllCount,
            patioAllLikes: this.state.patioAllLikes,
            patioImages: this.state.patioImages,
            patioCount: this.state.patioCount,
            patioLikes: this.state.patioLikes,
    ...
    handleSubmit(e) {
        ...
        this.state.patio = [...this.state.patio, {patioImage: this.state.patioPhoto, patioCount: 0, patioLikes: []}];
        this.setState({patio: this.state.patio});
        const patio = [];
        const bedroom = [];
        const kidsRoom = [];
        const livingRoom = [];
        ...
        const kitchenGarden = [];
        const diy = [];
        this.state.patio.map((design, id) => {
            const data1 = {};
            if (id !== this.state.patio.length) {
                data1["patioImage"] = design.patioImage + "";
            }


            if (id !== this.state.patio.length && design.patioCount !== undefined) {
    ...
    handleImageChange(e) {
        ...
        reader.onloadend = () => {
            ...
            this.state.patioPhoto = reader.result;
            this.state.patioImages.push(this.state.patioPhoto);
            this.setState({
                file: this.state.file,
                patioPhoto: this.state.patioPhoto,
                patioImages: this.state.patioImages,
            });
        };
    ...
    //setLoginDetails
    handleCount = (idx) => (evt) => {
        if (this.state.patioAllLikes[idx].includes(this.props.userName)) {
            this.state.patioAllCount[idx] = parseInt(this.state.patioAllCount[idx]) - 1;
            let index = this.state.patioAllLikes[idx].indexOf(this.props.userName);
            this.state.patioAllLikes[idx].splice(index, 1);
            this.setState({
                patioAllCount: this.state.patioAllCount,
                patioAllLikes: this.state.patioAllLikes,
            });
            ...
            this.state.patioAllCount[idx] = parseInt(this.state.patioAllCount[idx]) + 1;
            this.state.patioAllLikes[idx].push(this.props.userName);
            this.setState({
                patioAllCount: this.state.patioAllCount,
                patioAllLikes: this.state.patioAllLikes,
            });
        ...
        this.state.design.map((val, id) => {
            ...
            if (id !== this.state.design.length) {
                ...
                val.patio.map((design, idx) => {
                    ...
                    if (idx !== val.patio.length) {
                        data1["patioCount"] = this.state.patioAllCount[index] + "";
                    }


                    if (idx !== val.patio.length) {
                        data1["patioLikes"] = this.state.patioAllLikes[index];
                    }
                    index++;
                    patio.push(data1);
    ...
    render() {
        const favourite = (id) => {
            return ((this.state.patioAllLikes[id].includes(this.props.userName)) ?
...
export default withRouter(connect(mapStateToProps)(PatioDesign));
