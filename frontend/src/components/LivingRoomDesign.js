...
class LivingRoomDesign extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            file: '', livingRoomPhoto: '', livingRoomAllImages: [], livingRoomImages:[],
            livingRoomCount: [], livingRoomAllCount: [], livingRoomLikes: [], livingRoomAllLikes: []
        }
    }
    ...
    setDesignDetails = (result) => {
        ...
        this.state.design.map((val, id) => {
            if (val.livingRoom != null) {

                if (val.userName === this.props.userName) {
                    count=1;
                    val.livingRoom.map((design, id) => {
                        if (design.livingRoomImage !== null && design.livingRoomImage !== undefined) {
                            this.state.livingRoomImages.push(design.livingRoomImage);
                            if (id === 0) {
                                this.state.livingRoom[0].livingRoomImage = design.livingRoomImage;
                            }
                        }
                        if (design.livingRoomCount !== null && design.livingRoomCount !== undefined) {
                            this.state.livingRoomCount.push(design.livingRoomCount);
                            if (id === 0) {
                                this.state.livingRoom[0].livingRoomCount = design.livingRoomCount;
                            }
                        }
                        if (design.livingRoomLikes !== null && design.livingRoomLikes !== undefined) {
                            this.state.livingRoomLikes.push(design.livingRoomLikes);
                            if (id === 0) {
                                this.state.livingRoom[0].livingRoomLikes = design.livingRoomLikes;
                            }
                        }
                        if (id > 0) {
                            this.state.livingRoom = [...this.state.livingRoom, design];
                        }
                    })
                }
                val.livingRoom.map((design, id) => {
                    if (design !== null && design !== undefined) {
                        this.state.livingRoomAllImages.push(design.livingRoomImage);
                        this.state.livingRoomAllCount.push(design.livingRoomCount);
                        this.state.livingRoomAllLikes.push(design.livingRoomLikes);
                    }
                });
            }
        });
        if (this.state.livingRoom[0].livingRoomImage==="") {

            this.state.livingRoom.splice(0, 1);

        }
        console.log(this.state.livingRoom);
        console.log(this.state.livingRoomAllLikes);
        this.setState({
            livingRoomAllLikes: this.state.livingRoomAllLikes,
            livingRoomAllCount: this.state.livingRoomAllCount,
            livingRoomAllImages: this.state.livingRoomAllImages,
            livingRoomImages: this.state.livingRoomImages,
            livingRoomCount: this.state.livingRoomCount,
            livingRoomLikes: this.state.livingRoomLikes,
    ...
    handleSubmit(e) {
        ...
        this.state.livingRoom = [...this.state.livingRoom, {livingRoomImage: this.state.livingRoomPhoto,
            livingRoomCount: 0, livingRoomLikes: []}];
        ...
        const kitchenGarden = [];
        const diy = [];
        this.state.livingRoom.map((design, id) => {
            const data1 = {};
            if (id !== this.state.livingRoom.length) {
                data1["livingRoomImage"] = design.livingRoomImage + "";
            }
            if (id !== this.state.livingRoom.length && design.livingRoomCount !== undefined) {
    ...
    handleImageChange(e) {
        ...
        reader.onloadend = () => {
            ...
            this.state.livingRoomPhoto = reader.result;
            this.state.livingRoomImages.push(this.state.livingRoomPhoto);
            this.setState({
                file: this.state.file,
                livingRoomPhoto: this.state.livingRoomPhoto,
                livingRoomImages: this.state.livingRoomImages,
            });
        };
    ...
    //setLoginDetails
    handleCount = (idx) => (evt) => {
        if (this.state.livingRoomAllLikes[idx].includes(this.props.userName)) {
            this.state.livingRoomAllCount[idx] = parseInt(this.state.livingRoomAllCount[idx]) - 1;
            let index = this.state.livingRoomAllLikes[idx].indexOf(this.props.userName);
            this.state.livingRoomAllLikes[idx].splice(index, 1);
            this.setState({
                livingRoomAllCount: this.state.livingRoomAllCount,
                livingRoomAllLikes: this.state.livingRoomAllLikes,
            });
            ...
            this.state.livingRoomAllCount[idx] = parseInt(this.state.livingRoomAllCount[idx]) + 1;
            this.state.livingRoomAllLikes[idx].push(this.props.userName);
            this.setState({
                livingRoomAllCount: this.state.livingRoomAllCount,
                livingRoomAllLikes: this.state.livingRoomAllLikes,
            });
            console.log(this.state.livingRoomAllCount[idx]);
            console.log(this.state.livingRoomAllLikes[idx]);
        }
        ...
        this.state.design.map((val, id) => {
            ...
            if (id !== this.state.design.length) {
                ...
                val.livingRoom.map((design, idx) => {
                    const data1 = {};
                    if (idx !== val.livingRoom.length) {
                        data1["livingRoomImage"] = design.livingRoomImage + "";
                    }


                    if (idx !== val.livingRoom.length) {
                        data1["livingRoomCount"] = this.state.livingRoomAllCount[index] + "";
                    }


                    if (idx !== val.livingRoom.length) {
                        data1["livingRoomLikes"] = this.state.livingRoomAllLikes[index];
                    }
                    index++;
                    livingRoom.push(data1);
    ...
    render() {
        const favourite = (id) => {
            return ((this.state.livingRoomAllLikes[id].includes(this.props.userName)) ?
        ...
        let {livingRoomPhoto} = this.state;
        let $imagePreview = null;
        if (livingRoomPhoto) {
            $imagePreview = (<img className="img-design" src={livingRoomPhoto}/>);
        } else {
            $imagePreview = (<div className="previewText">Please select Living Room Design</div>);
        }
        return (
            <div className="main-container">
                ...
                <form className="container">
                    ...
                    <div className='card-list'>
                        <Label htmlFor="isbn" style={{fontStyle: 'oblique', fontSize: '30px'}}>Living Room Designs</Label>
                        <CardDeck>
...
export default withRouter(connect(mapStateToProps)(LivingRoomDesign));
